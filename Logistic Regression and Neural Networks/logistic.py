...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(0, max_iterations):
        P1 = np.dot(X,w) + b
        b1 = y - sigmoid(P1)
        w1 = (X.transpose()  * b1).transpose()
        w += step_size * ((1/float(N))*np.sum(w1, axis=0))
        b += step_size * ((1/float(N))*np.sum(b1, axis=0))
    assert w.shape == (D,)
    return w, b
...
def multinomial_predict(X, w, b):
    ...
    TODO: add your code here
    """   
    y_t = np.dot(X, w.transpose() ) + b
    assert y_t.shape == (N,C)
    preds = np.argmax(y_t, axis=1)                                                           
    assert preds.shape == (N,)
    return preds
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(0,C):
        y1 = (y==i)
        w[i], b[i] = binary_train(X, y1, w0, b0, step_size, max_iterations)
...
def OVR_predict(X, w, b):
    ...
    pred1 = np.zeros((C, N))
    for c in range(0, C):
        pred1[c] = binary_predict(X, w[c], b[c])
    preds = np.argmax(pred1, axis=0)
    assert preds.shape == (N,)
    return preds
