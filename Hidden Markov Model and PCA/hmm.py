...
def forward(pi, A, B, O):
  ...
  alpha[:,0]= pi*B[:,O[0]]
  for t in range(1,N):
      alpha[:,t] = B[:,O[t]]*np.dot(np.transpose(A),alpha[:,t-1])
  return alpha  
...
def backward(pi, A, B, O):
  ...
  for t in reversed(range(0,N-1)):
      beta[:,t] = np.dot(A,np.multiply(beta[:,t+1],B[:,O[t+1]]))
  return beta
...
def viterbi(pi, A, B, O):
  ...
  i[:,0] = [0,1]
  for t in range(1,N):
      delta[0,t] = np.max(delta[:,t-1]*A[:,0]*B[0,O[t]])
      i[0:,t] = np.argmax(delta[:,t-1]*A[:,0]*B[0,O[t]])
      delta[1,t] = np.max(delta[:,t-1]*A[:,1]*B[1,O[t]])
      i[0:,t] = np.argmax(delta[:,t-1]*A[:,0]*B[0,O[t]])
  i = i.astype(int)    
  path = np.zeros(N).astype(int)
  path[N-1] = np.argmax(delta[:,N-1])
  for k in reversed(range(0,N-1)):
      path[k] = i[path[k+1],k+1]
  path = path.tolist()
  return path
