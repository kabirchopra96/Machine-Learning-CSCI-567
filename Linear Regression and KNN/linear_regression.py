...
def regularized_linear_regression(X, y, lambd):
  ...
  Xtran = np.transpose(X)
  XtranX = np.dot(Xtran, X)
  XtranXlam = np.add(XtranX, np.dot(lambd, np.identity(len(X[0]))))
  XtranXlaminv = np.linalg.inv(XtranXlam)
  Xtrany = np.dot(Xtran, y)
  w = np.dot(XtranXlaminv, Xtrany)
  return w
...
def test_error(w, X, y):
  ...
  for i in range(len(y)):
    lineErr = np.dot(np.transpose(X[i]), w) - y[i]
    err = err + lineErr * lineErr
  return err/len(y)
  '''wtran=np.transpose(w)
  result = np.dot(X, wtran)
  error = np.square(np.subtract(y, result))
  err = np.sum(error)
  return err
'''
"""
