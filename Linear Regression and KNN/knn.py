...
def compute_distances(Xtrain, X):
	...
	for i in range (0,len(X1)) :
		dists.append([])
		for j in range(0,len(Xtrain1)) :
			dists[i].append(0)
			dists[i][j] = np.linalg.norm(X[i]-Xtrain[j])
				#dists[i][j]= dists[i][j] + ((X[i][k]-Xtrain[j][k])**2)
...
def predict_labels(k, ytrain, dists):
	...
	for i in range(0,len(dists1)) : 
		...
		for j in range(0,k) :
			ind[i].append(0) 
			ind[i][j]=dists1[i].index(min(dists1[i]))
			dists1[i][ind[i][j]]=dists1[i][ind[i][j]] + max(dists1[i])
			#for c in range(0,len(ind[i])) : 
			you[i].append(0)
			you[i][j]=ytrain[ind[i][j]]
		ypred[i]=max(you[i],key=you[i].count)
	return ypred
